 ...
 * 하단 CTA 문구.
 *
 * 동작은 촬영 진입이다 — 제거된 회색 자리표시 카드 3개(Figma 870:3635~3637)가 갖고 있던
 * 세안 확인 모달 진입점을 이 버튼이 승계한다(`uiStore.openWashCheck` → 세안 확인 → /camera).
 * 촬영 후 솔루션의 CTA(수행 완료)와 자리를 공유하는 슬롯이고, 규격도 그 버튼과 같다.
 *
 * 문구는 '오늘 피부 촬영하기' 에서 '제출하기' 로 교체했다(사용자 요청).
 * 촬영 후 모닝 프레임 최하단 버튼과 같은 자리·같은 규격이므로 문구도 그 버튼처럼
 * 짧은 동작 라벨로 둔다. **동작은 그대로다** — 라벨만 바뀌었다.
 *
 * 기본 솔루션에서는 수행 완료를 기록할 수 없다 — `careStore.completedDates` 는 촬영·분석을
 * 거친 솔루션의 수행 기록이고 캘린더 초록 칩의 근거이므로 여기에 섞으면 안 된다.
 */
export const BASIC_CTA_LABEL = '제출하기';
