...
| `SkinAnalysisModal.jsx` | "데일리 스킨 분석" modal (Figma `1252:493`) — replaces a static WHS-app screenshot pasted into that Figma frame with a real vector `SkinRadarChart` in the app's own accent color (not WHS's yellow). Originally planned as the 5 AI-photo-analysis factors from `AAC_AI_Skin_Care_Service_Planning.md` §8.1, but the backend's actual `MetricScores` field merges pore+texture into one `poreTextureScore` (no separate texture score) — so it's a **4-axis** chart (pigmentation/poreTexture/wrinkle/redness); `constants/skinAnalysis.js`/`SkinRadarChart` are axis-count-agnostic, so adding a 5th+ real factor later just makes it a pentagon/hexagon automatically. Real scores come from `hooks/useSkinAnalysisScores.js`; falls back per-factor to `constants/skinAnalysis.js`'s dummy scores when there's no session/no analysis yet/a failed request. Driven by `uiStore`'s `skinAnalysisOpen`; mounted globally in `layout/AppModals.jsx`. `RoutineScreen.jsx` auto-opens it once via router `location.state.showSkinAnalysis` right after the solution-generation flow, and offers a floating reopen button (no Figma spec — ad hoc placement) for later. |
...
### Internal
`@/i18n` (`useT`), `@/lib/useMountTransition`, `@/store/uiStore` (`skinAnalysisOpen`), `@/constants/skinAnalysis` (`SKIN_ANALYSIS_FACTORS`), `@/hooks/useSkinAnalysisScores`, `@/assets/figma/routine-notch.svg`, `@/assets/figma/routine-right.svg`, `@/assets/figma/routine-time.svg`, `@/assets/figma/step-divider.svg`.
