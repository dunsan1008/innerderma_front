...
| `cartStore.js` | `useCartStore` — MY cart state: `items` (with quantity/delivery), `selectedIds` (for bulk purchase/delete). Actions: `toggleSelect`, `toggleSelectAll`, `setQuantity`, `setDelivery`, `setDeliveryForSelected`, `remove`, `removeSelected`, `add` (merges quantity if already present), `selectedTotal()`, `clear`. Persisted at version 2 — migration wipes any v1 dummy-seeded cart so real users always start empty. |
