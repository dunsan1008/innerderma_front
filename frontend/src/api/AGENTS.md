...
| `market.js` | `getProducts({category,concern})`, `getProduct`, `getKnowledgeProducts`, `getKnowledgeProduct`, `getDailyProductRecommendations`. **Flagged in-file**: no explicit Pith/WIM store field on a product (probably `brand`, unconfirmed), and recommendations aren't split per store/category tab. |
