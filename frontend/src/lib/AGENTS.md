...
| `syncBackendCollections.js` | `syncBackendCollections(userCode)` — fired (not awaited) from `SplashScreen`'s returning-user branch. Fetches the real cart + wishlist, resolves each `productId` to a full product via `api/market.js`'s `getProduct` (deduped, one lookup per unique code across both lists), and calls `cartStore`/`wishlistStore`'s `mergeFromServer` to backfill local state on a fresh session — this is the pull side of the write-only sync those stores otherwise do. Never throws; failures are console-only so a broken lookup can't block Splash's navigation. |
| `userCode.js` | `generateUserCode()` — `"USER-" + crypto.randomUUID()`. The backend has no password-based auth; a client-generated userCode is the entire credential, minted once at first registration (see `pages/ConnectingScreen.jsx`, `api/auth.js`) and reused via `store/authStore.js` on every later visit. |
